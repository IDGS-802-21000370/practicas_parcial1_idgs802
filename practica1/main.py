...
def indexDiccionario():
    form_dicc = form_diccionario.diccionarioForm(request.form)
        
    return render_template('diccionario.html', form = form_dicc)

@app.route('/diccionario/guardar', methods=["GET","POST"])
def guardarDiccionario():
    if request.method == "POST":
        form_dicc = form_diccionario.diccionarioForm(request.form)
        ing = form_dicc.ingles.data
        es = form_dicc.español.data
...
def buscarDiccionario():
    if request.method == "POST":
        form_dicc = form_diccionario.diccionarioForm(request.form)
        buscar = form_dicc.buscar.data
        traduccion = form_dicc.radio.data
        
        if traduccion == 'español':
            print('es')
            valor = buscar_valor(buscar)
        elif traduccion == 'ingles':
            print('ing')
            valor = buscar_key(buscar)
        
        return render_template('diccionario.html', form = form_dicc, valorEncontrado = valor)
